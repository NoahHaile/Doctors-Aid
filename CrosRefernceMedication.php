<html lang="en">
  <head>
    ...
    <title>Cross Reference Medication</title>
    <link
      rel="stylesheet"
...
<button class="del" onclick="hideModal()">reply</button>
    ...
    <section class="feedback" id="feedback"> 
      <h2>search type</h2>
       ...
       <input type="radio" id="inc" value="Inclusive" checked name="search-type">Inclusive
        <input  class="ex" type="radio" id="exc" value="Exclusive" name="search-type">Exclusive
      </div>
      ...
      <!-- <input class="search" type="text" placeholder="" id="select" name="search" />
            
          <input type="submit"  id="del" class="del" value="Delete" class="btn" /> -->
         <div class="row">
          
           <form  class="fone" action="">
             <div class="lbl"><label class="lbl" for="bd">Disease</label></div>
       ...
       <form class="mess" action="">
            <h3 class="opt"> medication Result</h3>
            <textarea name="" id="result" cols="30" rows="10" disabled></textarea>
       </form>
         </div>
...
fetch('http://localhost/Doctor Aidv11/core/crm.php', {
        method: "POST",
        headers: {
      ...
      .then((res) => {
        console.log(res);
        if(res==''){
result.value='no medication found in the Database';
        } else{
result.value='';
