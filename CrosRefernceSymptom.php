<html lang="en">
  <head>
    ...
    <title>Cross Referance Symptom</title>
    <link
      rel="stylesheet"
...
<button class="del" onclick="hideModal()">reply</button>
    ...
    <section class="feedback" id="feedback"> 
      <h2>search type</h2>
       ...
       </label><input type="radio" id="inc" value="Inclusive" checked name="search-type">Inclusive
        <input  class="ex" type="radio" value="Exclusive" id="exc" name="search-type">Exclusive
      </div>
      ...
      <!-- <input class="search" type="text" placeholder="" name="search" id="select"/>      
         ...
         <form  class="fone" action="">
             <div class="lbl"><label class="lbl" for="bd">Symptom</label></div>
       ...
       <form class="mess" action="">
            <h3 class="opt"> Possible Disease</h3>
...
fetch('http://localhost/Doctor Aidv11/core/crs.php', {
        method: "POST",
        headers: {
      ...
      .then((res) => {
        console.log(res);
        if(res==''){
result.value='no possible disease found in the Database';
        } else{
result.value='';
