...
<div class="error-msg msg">
 ...
 It is not successful!
        ...
        <div class="row">
        <form name="addMed" method="post" action="core/cds.php" >
                <h3>Add <span>Disease</span></h3>
                ...
                <!-- <input type="submit" value="Add Medication" class="btn btnmsg1"> -->
                <input type="submit" name="sub1" value="Add Disease" class="btn btnmsg1" id="add">
            </form>

            <form action="core/cds.php" method="post" class="form2 ed" name="form2">
                <h3>Edit <span>Disease</span></h3>
                ...
                if(!sqlsrv_execute($stmt))
                  ...
                  $deseases[$num++] = $row['Dname'];
                  $deseases[$num++] = $row['Ddescription'];
                  }
                ?>
                  </select>
                <div class="lbl"><label class="lbl" for="addName">Name</label></div>
                ...
                <input type="submit" value="Update Disease" class="btn btnmsg1" name="sub2">
            </form>


            <form  action="core/cds.php" method="post" >
            ...
            <form  action="core/cds.php" method="post">
                ...
                <option value="" disabled selected hidden>Select Cross Referance</option>
                 <?php
                $sql = "EXEC [dbo].[getAllCrossDisease]";
                $stmt = sqlsrv_prepare($conn, $sql, array());
                if(!sqlsrv_execute($stmt))
                    die( print_r( sqlsrv_errors(), true));
                    $did = array();
                    while($row = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_ASSOC)){
                    echo  "<option value='".$row['SID']." ".$row['Did']."'>".$row['Dname']." Exhibits ".$row['Sname']."</option>";
            ...
            <form action="core/cds.php" method="post" class="more">
                ...
                <option value="" disabled selected hidden>Select Symptom</option>
                <?php
                $sql = "EXEC [dbo].[getAllSymptom]";
                $stmt = sqlsrv_prepare($conn, $sql, array());
                if(!sqlsrv_execute($stmt))
                    die( print_r( sqlsrv_errors(), true));
                    while($row = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_ASSOC)){
  ...
  <script src="js/dark_mode.js"></script>  
   <script src="js/delete.js"></script> 
    ...
    if(dmode=='dark'){
      ...
      <script>
    var js_arr = '<?php echo JSON_encode($deseases);?>';
